...
// For now, these are STUBS that fall back to mock data.
// When the backend is ready, remove the mock fallbacks.
// ============================================================
...
export async function getEspacioDetalle(espacioId: string): Promise<EspacioDetalle> {
  if (USE_MOCK) {
    ...
    const { mockSharedMembers, mockSharedTransactions } = await import("@/lib/mock-data")
