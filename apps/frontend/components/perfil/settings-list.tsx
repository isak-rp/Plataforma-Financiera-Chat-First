...
import { UserPen, Bell, Coins, Moon, Info, LogOut, ChevronRight } from "lucide-react"
  ...
  return (
    <div className="flex flex-col">
      {items.map((item, index) => (
        ...
        >
          <item.icon
            className={`h-5 w-5 ${item.destructive ? "text-destructive" : "text-muted-foreground"}`}
          />
          <span
          ...
          {!item.destructive && <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </button>
      ))}
    </div>
  )
